<template>
    ...
    <div class="save">
        ...
        <el-button @click="initDrawTubes(scene)">管道</el-button>
            ...
            if (intersect.object) {
                if (selectObject) {
                    ...
                    const tubes = scene.children.filter(item => 
                        item.userData?.referTube?.find(item => item === name)
                    );
                    tubes.forEach(item => {
                        scene.remove(item);
                        //dispose
...
function drawTubes(scene, models, radius=5, color) {
